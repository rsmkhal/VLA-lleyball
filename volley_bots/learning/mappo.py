...
class MAPPOPolicy(object):
    ...
    def load_state_dict(self, state_dict):
        ...
        if "value_normalizer" in state_dict and hasattr(self, "value_normalizer"):
            try:
                vn_state = state_dict["value_normalizer"].copy()
                current_vn_state = self.value_normalizer.state_dict()

                patched_vn_state = {}
                for k, cur in current_vn_state.items():
                    if k not in vn_state:
                        patched_vn_state[k] = cur
                        continue

                    old = vn_state[k].to(cur.device, cur.dtype)

                    if old.shape == cur.shape:
                        patched_vn_state[k] = old
                    elif old.numel() == 1:
                        # scalar-like stat -> broadcast
                        patched_vn_state[k] = torch.full_like(cur, old.item())
                    elif old.ndim == cur.ndim and all(
                        od == cd or od == 1 for od, cd in zip(old.shape, cur.shape)
                    ):
                        patched_vn_state[k] = old.expand_as(cur).clone()
                    else:
                        # fallback: keep current tensor
                        patched_vn_state[k] = cur
                        print(
                            f"[Checkpoint] value_normalizer key skipped: {k} {tuple(old.shape)} -> {tuple(cur.shape)}"
                        )

                self.value_normalizer.load_state_dict(patched_vn_state, strict=False)
                print("[Checkpoint] value_normalizer loaded (shape-safe)")
